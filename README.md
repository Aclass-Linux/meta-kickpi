...
- [ ] 阶段 A：建立 `meta-kickpi` layer
- [ ] 阶段 B：建立 K7 machine 配置
...
- [x] `git remote -v` 指向 `Aclass-Linux/meta-kickpi.git`
- [x] 当前分支为 `scarthgap`
- [ ] `git status` 干净
- [ ] GitHub 上能看到包含最新 layer 配置和迁移文档的提交
...
- [ ] `repo list` 显示 `sources/meta-kickpi`
- [ ] 单项目 `repo sync` 成功
- [ ] `repo status` 没有意外修改
...
- [ ] 新工作区可通过 `repo init` 和 `repo sync` 取得 `meta-kickpi`
- [ ] 已删除重复的 local manifest
- [x] `meta-kickpi` layer 已创建并推送 `scarthgap` 分支
- [ ] 正式 K7 manifest 已提交、推送并通过 `repo sync` 验证
...
```bash
MACHINE=kickpi-k7 bitbake -e virtual/kernel | grep -E '^(MACHINE|KERNEL_DEVICETREE|PREFERRED_PROVIDER_virtual/kernel)='
```

完成标准：

- [ ] BitBake 能识别 `kickpi-k7`
- [ ] kernel provider 为 `linux-rockchip`
- [ ] `KERNEL_DEVICETREE` 指向 K7 DTB

## 4. 阶段 C：迁移 K7 Linux 设备树

### C1. 收集 K7 DTS

至少检查以下文件及其 `#include` 依赖：
...
不要直接复制编译后的 DTB。设备树应随内核源码编译。

### C2. 生成内核补丁

推荐在 Yocto 实际使用的 Linux 源码基线上添加 DTS，然后用 Git 生成补丁。目标结构：
...
SRC_URI:append:kickpi-k7 = " file://0001-arm64-dts-rockchip-add-kickpi-k7.patch"
```

补丁必须同时修改内核 DTS Makefile，使其生成：
...
- [ ] 提交并推送正式 K7 manifest，然后执行 `repo sync`
- [ ] 创建 `sources/meta-kickpi/conf/machine/kickpi-k7.conf`
- [ ] 运行 `MACHINE=kickpi-k7 bitbake -e virtual/kernel`
